...
const { parseJSON } = require('../helpers/utiliteis');
...
worker.validateCheckData = (originalCheckData) => {
    ...
    if (originalCheckData && originalCheckData.id) {
        ...
        console.log('Error : check was invalid or not properly formateed');
    }
};
...
// save the outcome to database and sent to next process
worker.processCheckOutcome = (originalCheckData, checkOutCome) => {
...
// timer to ecxcute the worker porcess once per minute
worker.loop = () => {
    setInterval(() => {
        worker.gatherAllChecks();
    }, 1000 * 6);
